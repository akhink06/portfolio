...
export const Circle = ({ className, children, idx, style }) => {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
export const IconContainer = ({ icon, text, delay }) => {
  return (
    ...
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5 shadow-inner backdrop-blur-sm">
        {icon || (
      ...
      <div className="rounded-md px-2 py-1">
        <div className="text-center text-[0.65rem] font-bold uppercase tracking-[1.5px] text-slate-400">
          {text || "Tech"}
        </div>
      </div>
    </motion.div>
